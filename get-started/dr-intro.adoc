...
summary: La recuperación ante desastres en la nube es una forma resistente y rentable de proteger las cargas de trabajo frente a interrupciones en el sitio y eventos de corrupción de datos. Con NetApp Disaster Recovery, puedes replicar tus cargas de trabajo VMware locales que ejecutan almacenamiento ONTAP a un centro de datos VMware definido por software en una nube pública usando almacenamiento basado en cloud de NetApp o a otro entorno VMware local con almacenamiento ONTAP como sitio de recuperación ante desastres. 
---
= Obtenga más información sobre la NetApp Disaster Recovery
:hardbreaks:
:allow-uri-read: 
:icons: font
...
[role="lead"]
La recuperación de desastres en la nube es una forma resistente y rentable de proteger las cargas de trabajo frente a interrupciones del sitio y eventos de corrupción de datos. NetApp Disaster Recovery es un servicio de recuperación de desastres basado en cloud que automatiza los flujos de trabajo de recuperación de desastres. Con NetApp Disaster Recovery, puedes proteger tus clústeres de Kubernetes locales, cargas de trabajo basadas en NFS y almacenes de datos del sistema de archivos de máquinas virtuales (VMFS) de VMware vSphere para iSCSI y FC que ejecutan almacenamiento NetApp hacia uno de los siguientes:

* Amazon Elastic VMware Service (EVS) con Amazon FSx for NetApp ONTAP. Para obtener más información, consulta link:../reference/evs-deploy-guide-introduction.html["Introducción a la recuperación ante desastres de NetApp con Amazon Elastic VMware Service y Amazon FSx for NetApp ONTAP"].
* VMware Cloud (VMC) en AWS con Amazon FSx for NetApp ONTAP
* Solución de Azure VMware con NetApp Cloud Volumes ONTAP (iSCSI) (versión preliminar privada)
...
image:../use/evs-console-architecture.png["Diagrama de los componentes de NetApp Disaster Recovery"]
...
== Ventajas de utilizar NetApp Disaster Recovery
...
== Capacidades de NetApp Disaster Recovery
...
* Realiza una prueba de conmutación por error. Cuando haces esto, el servicio crea máquinas virtuales temporales. Disaster Recovery crea un nuevo volumen FlexClone a partir de la instantánea seleccionada, y un almacén de datos temporal, que está respaldado por el volumen FlexClone, se asigna a los hosts ESXi. Este proceso no consume capacidad física adicional en el almacenamiento ONTAP local ni en FSx para NetApp ONTAP storage en AWS. El volumen de origen original no se modifica y los trabajos de réplica pueden continuar incluso durante la recuperación de desastres.
...
image:../use/evs-use-cases.png["Diagrama que muestra casos de uso para NetApp Disaster Recovery"]


NOTE: Debes configurar el servidor vSphere fuera de NetApp Disaster Recovery en vSphere Server.
...
* Adquiere una suscripción de pago por uso (PAYGO) a través de Amazon Web Services (AWS) Marketplace, Azure Marketplace o Google Cloud Marketplace. Esta licencia te permite adquirir una licencia de capacidad protegida fija sin ningún compromiso a largo plazo.
...
Disaster Recovery se licencia según la cantidad de datos alojados en volúmenes ONTAP protegidos. El servicio determina qué volúmenes deben considerarse para fines de licenciamiento asignando las máquinas virtuales protegidas a sus almacenes de datos vCenter. Cada almacén de datos se aloja en un volumen o LUN de ONTAP. La capacidad utilizada informada por ONTAP para ese volumen o LUN se usa para las determinaciones de licenciamiento.
...
NOTE: NetApp los cargos por recuperación ante desastres se basan en la capacidad utilizada de los almacenes de datos del sitio de origen cuando hay al menos una máquina virtual o un recurso de Kubernetes que tiene un plan de replicación. La capacidad de un almacén de datos sometido a conmutación por fallo no se incluye en la asignación de capacidad. Para un BYOL, si los datos superan la capacidad permitida, las operaciones en el servicio se limitan hasta que obtengas una licencia de capacidad adicional o actualices la licencia en la NetApp Console.
...
Puedes probar NetApp Disaster Recovery con una prueba gratuita de 30 días.

Para continuar después de los 30 días de prueba, necesitas obtener una suscripción de pago por uso (PAYGO) de tu proveedor de cloud o comprar una licencia BYOL de NetApp.
...
image:dr-architecture-diagram-70-2.png["Diagrama que muestra la arquitectura de la infraestructura del servicio NetApp Disaster Recovery"]
...
Disaster Recovery utiliza los siguientes componentes:
...
* VMware Cloud (VMC) en AWS con Amazon FSx for NetApp ONTAP
* Otro entorno VMware local basado en NFS con almacenamiento ONTAP o un entorno VMFS local FC/iSCSI
