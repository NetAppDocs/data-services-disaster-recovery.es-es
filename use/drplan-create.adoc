...
[role="lead"]
Una vez que hayas añadido recursos y sitios, crea un _plan de replicación_ para proteger tus datos. El plan define los sitios de origen y destino, los grupos de recursos, las asignaciones de recuperación y el comportamiento de encendido de tus aplicaciones. También puedes configurar programas de cumplimiento y de pruebas, y realizar pruebas de conmutación por error sin afectar los datos de producción.
...
[[about-replication-plans]]
== Acerca de los planes de replicación

Puedes proteger varias máquinas virtuales en varios almacenes de datos. NetApp Disaster Recovery crea grupos de consistencia de ONTAP para todos los volúmenes de ONTAP que alojan almacenes de datos de máquinas virtuales protegidas.
...
** Si faltan instantáneas en el sitio de destino, es posible que el servicio elimine las instantáneas correspondientes del sitio de origen durante el siguiente proceso programado de conciliación de instantáneas, dependiendo de la política predeterminada de SnapMirror para la relación.
...
[[considerations-for-platform-managed-backups]]
=== Aspectos a tener en cuenta en las copias de seguridad gestionadas por la plataforma

Si tienes previsto usar copias de seguridad gestionadas por la plataforma (es decir, instantáneas gestionadas por ONTAP), se recomienda que uses la política de replicación *MirrorAll*. Revisa la tabla para entender el impacto de las diferentes políticas de replicación de SnapMirror y por qué MirrorAll es la opción recomendada.

[cols="10,10,80"]
|===


| Política | Recomendación | Notas 


| *MirrorAll* | Recomendado | No utilices instantáneas de transferencia de SnapMirror para operaciones de conmutación por error o restablecimiento. 


| *MirrorVault* .2+| Recomendación alternativa .2+| Asegúrate de que las copias de Snapshot seleccionadas durante la conmutación por error o la conmutación por recuperación estén presentes tanto en el volumen de origen como en el de destino. De lo contrario, se producirá el error: "Transition to 'snapmirrored' state failed. Reason: No common snapshot found." No utilices instantáneas de transferencia de SnapMirror para operaciones de conmutación por error o restablecimiento. 


| *Asíncrono* 


| *MirrorLatest* | _NO_ se recomienda | Esta política hace que el destino disponga, en la práctica, de una única instantánea común para la conmutación por error, lo cual no es óptimo para situaciones de recuperación ante desastres. 
|===


[[considerations-for-disaster-recovery-managed-backups]]
=== Aspectos a tener en cuenta para las copias de seguridad gestionadas por NetApp Disaster Recovery

Al utilizar las copias de seguridad gestionadas de NetApp Disaster Recovery, se aplican las siguientes directrices a las relaciones de SnapMirror creadas fuera de NetApp Disaster Recovery:

* Las relaciones de SnapMirror pueden admitir las políticas de replicación MirrorAll, MirrorVault, MirrorLatest y Asynchronous.
* *No* programes actualizaciones para estas relaciones, ya que serán gestionadas por el servicio de NetApp Disaster Recovery.
...
TIP: Organiza tus máquinas virtuales o clústeres de Kubernetes antes de implementar NetApp Disaster Recovery para minimizar la dispersión. Coloca los recursos que necesitan protección en un subconjunto de almacenes de datos y los recursos que no se van a proteger en un subconjunto diferente de almacenes de datos. Para organizar grupos de recursos antes de crear el plan de replicación, consulta link:resource-group.html["Crear un grupo de recursos"].
...
El asistente te guía a través de estos pasos:
...
+
image:dr-plan-create-name2.png["Captura de pantalla que muestra la página del servidor vCenter"]
...
* No alojes recursos protegidos y no protegidos en el mismo almacén de datos. Si alojas recursos protegidos y no protegidos en el mismo almacén de datos, puedes encontrarte con estos problemas:
+
...
** Si es necesario realizar una conmutación por error del grupo de recursos y sus recursos asociados al sitio de recuperación ante desastres, cualquier recurso no protegido dejará de existir en el sitio de origen a raíz del proceso de conmutación por error, lo que provocará el fallo de dichos recursos en el sitio de origen. Además, NetApp Disaster Recovery no iniciará esos recursos no protegidos en el sitio de conmutación por error.





TIP: Crea un conjunto separado de asignaciones dedicadas para tus pruebas de conmutación por error para evitar que los recursos se conecten a las redes de producción usando las mismas direcciones IP.
...
+
** Para eliminar una máquina virtual o un almacén de datos, pasa el cursor por encima del nombre del recurso y luego selecciona *X*.
** Para organizar los recursos en varios grupos de recursos, selecciona *Haz clic para añadir otro grupo de recursos*. Una vez añadido el grupo de recursos, puedes arrastrar y soltar recursos entre grupos. Selecciona el icono del lápiz para editar el nombre del grupo de recursos.
+
...
+
Si no tienes grupos de recursos existentes o necesitas añadir recursos a uno, selecciona *+ Crear nuevo grupo de recursos*. Sigue las instrucciones para link:resource-group.html["agrega espacios de nombres a un grupo de recursos"].
...
TIP: Ejecuta el comando CLI `kubectl get pvc -n <namespace>` en el espacio de nombres para ver la clase de almacenamiento.
...
+
Al configurar la frecuencia de las instantáneas, debes elegir una frecuencia de al menos cada cinco minutos.
...
+
Para Azure, elige la *suscripción*, el *grupo de recursos* y la *cuenta de almacenamiento* para el proveedor de la nube.

+
Para AWS, ONTAP S3 y StorageGRID, selecciona la *cuenta*, las *credenciales* y el *bucket*.
...
.. Selecciona **Añadir**.
+
image:kubernetes-mapping.png["Captura de pantalla de la asignación de clases de almacenamiento para Kubernetes"]
...
==== Transformaciones de recursos y ejecuciones de scripts

En los planes de replicación de Kubernetes, puedes aplicar transformaciones de recursos y ejecuciones de scripts. Estos pasos son opcionales.

.Agrega una transformación de recursos
La incorporación de una transformación de recursos permite modificar cualquier aspecto de una carga de trabajo durante la conmutación por error. Las transformaciones de recursos ofrecen flexibilidad para adaptar las cargas de trabajo a diferentes entornos y configuraciones durante un evento de recuperación ante desastres. link:task-resource-transformation.html["Aprende a crear transformaciones y a asignarles recursos."^]

Las transformaciones de recursos se aplican en el orden en que las indiques.

. En el menú desplegable, selecciona la transformación de recursos que quieras añadir.
. Elige a qué recursos quieres aplicar la transformación y luego añade los valores necesarios
. Revisa el orden de las transformaciones para asegurarte de que las operaciones se ejecuten en el orden adecuado.


.Agrega una ejecución de script
La recuperación ante desastres puede ejecutar un script antes o después de tomar una instantánea para gestionar la suspensión y la reanudación de una aplicación. Realizar esta acción garantiza que las aplicaciones estén debidamente preparadas antes de que se replique y restaure una instantánea y que puedan reanudar sus operaciones sin problemas después de la conmutación por error.

. Selecciona *+ Add script*.
. Selecciona *Cargar* para elegir el archivo de script.
. Elige la *fase de ejecución*: *Pre-snapshot* para ejecutar antes de crear la instantánea o *Post-snapshot* para ejecutar después.
. Seleccione *Guardar*.
...
La sección «Recursos de computación» define dónde se restauran las máquinas virtuales tras una conmutación por error. Asocia el centro de datos y el clúster de origen vCenter a un centro de datos y un clúster de destino.

Opcionalmente, las máquinas virtuales se pueden reiniciar en un host ESXi específico vCenter. Si VMware DRS está habilitado, puedes trasladar la máquina virtual a un host alternativo de forma automática si es necesario para cumplir con la política de DR configurada.

Si lo deseas, puedes colocar todas las máquinas virtuales de este plan de replicación en una carpeta única dentro del inventario de vCenter. Esto te da una forma fácil de organizar rápidamente las máquinas virtuales con conmutación por error dentro de vCenter.
...
NOTE: Esta sección sólo es necesaria para vCenters. No es necesario completarla para Kubernetes.
...
En la pestaña «Asignaciones de conmutación por error», selecciona la flecha hacia abajo situada junto a *Redes virtuales*. Selecciona la red virtual de origen y la red virtual de destino.
...
NOTE: Activa *Configuración avanzada* para configurar *Scripts* y *consistencia con las aplicaciones*.
...
* *Tipo de dirección IP*: Reconfigura la configuración de la máquina virtual para que se ajuste a los requisitos de la red virtual de destino. NetApp Disaster Recovery ofrece dos opciones: DHCP o IP estática. Para las IPs estáticas, configura la máscara de subred, la puerta de enlace y los servidores DNS. Además, introduce las credenciales para las máquinas virtuales.
+
...
+
*Máquinas virtuales que ejecutan Microsoft Windows*: pueden ejecutar scripts por lotes de Windows (.bat) o PowerShell (ps1). Los scripts de Windows pueden utilizar argumentos de línea de comandos. Da formato a cada argumento en el formato `arg_name$value`, donde `arg_name` es el nombre del argumento y `$value` es el valor del argumento, y un punto y coma separa cada par `argument$value`.
...
* *Conservar la jerarquía de carpetas original*: de manera predeterminada, Disaster Recovery conserva la jerarquía de inventario de VM (estructura de carpetas) en caso de conmutación por error. Si el destino de recuperación _no_ tiene la jerarquía de carpetas original, Disaster Recovery la crea.
+
...
+
** Arranque secuencial: Asigna a cada máquina virtual un número único para que se inicien en el orden asignado, por ejemplo, 1,2,3,4,5.
...
image:dr-plan-resource-mapping-vm-boot-order.png["Captura de pantalla que muestra la sección de la máquina virtual de asignaciones de recursos"]
...
Los datos gestionados por el servidor de bases de datos pueden encontrarse en el mismo almacén de datos que la máquina virtual que aloja dicho servidor, o bien en un almacén de datos diferente. La siguiente tabla muestra las configuraciones compatibles con las instantáneas consistentes con las aplicaciones en NetApp Disaster Recovery:
...
* En el caso de las máquinas virtuales de Windows, el servicio utiliza el Servicio de copias en la sombra de volumen (VSS) de Microsoft para coordinarse con cualquiera de los dos tipos de servidor de bases de datos.
...
TIP: Consulta la <<Aspectos a tener en cuenta en las copias de seguridad gestionadas por la plataforma>> antes de realizar estas selecciones.

* *Utiliza copias de seguridad y programas de retención gestionados por la plataforma*: Si estás usando una solución externa de gestión de instantáneas, marca esta casilla. NetApp Disaster Recovery admite el uso de soluciones externas de gestión de instantáneas, como el programador de políticas nativo de ONTAP SnapMirror o integraciones de terceros. Si cada datastore (volumen) del plan de replicación ya tiene una relación SnapMirror que se gestiona en otro lugar, puedes usar esas instantáneas como puntos de recuperación en NetApp Disaster Recovery.
+
Cuando se selecciona esta opción, NetApp Disaster Recovery no configura una programación de copias de seguridad. Sin embargo, todavía necesitas configurar una programación de retención porque es posible que se sigan tomando instantáneas para pruebas, conmutación por error y conmutación de retorno.
...
* *Almacenes de datos de origen y de destino*: Si existen varias relaciones SnapMirror (tipo fan-out), puedes seleccionar el destino que quieres usar. Si un volumen ya tiene establecida una relación SnapMirror, aparecen los almacenes de datos de origen y de destino correspondientes. Si un volumen no tiene una relación SnapMirror, puedes crear una ahora seleccionando un clúster de destino, seleccionando una SVM de destino y proporcionando un nombre para el volumen. El servicio creará el volumen y la relación SnapMirror.
+
...
** Durante la conmutación por error, puedes seleccionar cualquier instantánea. Si seleccionas la instantánea más reciente, el servicio crea una copia de seguridad bajo demanda, actualiza el destino y utiliza esa instantánea para la conmutación por error.
