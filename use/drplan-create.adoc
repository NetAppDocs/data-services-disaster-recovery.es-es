...
* *Conservar la jerarquía de carpetas original*: cuando se selecciona, la recuperación ante desastres conserva la jerarquía de inventario de la máquina virtual (estructura de carpetas) en caso de conmutación por error. Si el destino de recuperación _no_ tiene la jerarquía de carpetas original, se crea la recuperación ante desastres. Marque la casilla para conservar la jerarquía de carpetas; desmárquela para ignorar la jerarquía de carpetas original.
...
TIP: Se recomienda crear una política de exportación dedicada que limite el acceso al volumen _únicamente_ a los hosts vCenter ESXi de origen y destino que alojarán las máquinas virtuales protegidas. Esto garantiza que entidades externas no puedan acceder a la exportación NFS.
