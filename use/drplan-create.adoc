...
. En el menú NetApp Disaster Recovery, selecciona *Replication plans* y selecciona *Add*. Si es la primera vez que creas un plan de replicación, desde el Dashboard, selecciona *Add replication plan*.
. Ingresa un nombre para el plan de replicación.
. En *Source vCenter*, selecciona el vCenter donde existen los datos en el menú desplegable. En *Target vCenter*, selecciona el vCenter al que quieres replicar los datos en el menú desplegable.
+
...
* Asegúrate de que hay al menos una máquina virtual en el datastore. Si no hay máquinas virtuales en el datastore, el datastore no se detectará.
* Un único almacén de datos no debe alojar máquinas virtuales protegidas por más de un plan de replicación.
* No alojes máquinas virtuales protegidas y desprotegidas en el mismo almacén de datos. Si las máquinas virtuales protegidas y desprotegidas están alojadas en el mismo almacén de datos, puedes encontrarte con estos problemas:
+
** La capacidad utilizada de ese volumen puede contribuir al cálculo de licencias porque Disaster Recovery utiliza SnapMirror, lo que significa que el sistema replica volúmenes ONTAP completos. En este caso, el espacio de volumen consumido tanto por las VMs protegidas como por las no protegidas se incluiría en este cálculo.
...
TIP: Cree un conjunto dedicado e independiente de asignaciones para sus pruebas de conmutación por error para evitar que los VMS se conecten a redes de producción utilizando las mismas direcciones IP.
...
* *CPU* y *Memoria (GB)*: en los detalles de las máquinas virtuales, puedes cambiar opcionalmente el tamaño de la CPU y .
+
