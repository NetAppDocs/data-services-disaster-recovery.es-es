...
[role="lead"]
Después de que hayas añadido los recursos y los sitios, ya estás listo para crear un _plan de replicación_. Los planes de replicación gestionan la protección de datos. Crear la replicación implica designar los sitios de origen y destino, seleccionar los grupos de recursos y elegir cómo deben restaurarse y encenderse las aplicaciones. Por ejemplo, podrías agrupar máquinas virtuales (VM) asociadas con una aplicación o podrías agrupar aplicaciones que tengan niveles similares.
...
. Para *Source*, selecciona el clúster vCenter o Kubernetes que quieras designar como origen.
...
** Si el grupo de recursos y sus recursos asociados necesitan ser traspasados al sitio de recuperación de desastres, cualquier recurso desprotegido dejará de existir en el sitio de origen desde el proceso de conmutación por error, lo que provocará el fallo de las máquinas virtuales desprotegidas en el sitio de origen. Además, NetApp Disaster Recovery no iniciará esas máquinas virtuales desprotegidas en el sitio de conmutación por error vCenter.
...
.Pasos
. Si ya tienes grupos de recursos, elígelos de la lista.
+
